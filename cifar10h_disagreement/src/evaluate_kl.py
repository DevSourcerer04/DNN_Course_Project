...
def plot_entropy_scatter(true_entropy, pred_entropy):
    ...
    plt.title("Predicted vs True Entropy - KL Model")
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "kl_predicted_vs_true_entropy.png")
...
def main():
    ...
    model_path = os.path.join(MODELS_DIR, "resnet18_kl_best.pth")
    ...
    save_csv = os.path.join(RESULTS_DIR, "kl_test_results.csv")
    ...
    pred_save_path = os.path.join(RESULTS_DIR, "kl_test_predictions.npz")
    np.savez(
        pred_save_path,
        indices=indices,
