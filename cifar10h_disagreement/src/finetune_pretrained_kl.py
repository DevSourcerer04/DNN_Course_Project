...
def plot_training_curves(train_losses, val_losses, val_entropy_mae):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("KL Loss")
    plt.title("CIFAR-10 Pretrained → CIFAR-10H Fine-tuning Loss")
    plt.legend()
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "pretrained_finetune_kl_loss.png")
    ...
    plt.title("Pretrained Fine-tuning Entropy Error")
    plt.legend()
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "pretrained_finetune_entropy_mae.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
    print("Saved:", save_path)
...
def main():
    ...
    pretrained_path = os.path.join(MODELS_DIR, "resnet18_cifar10_pretrained.pth")
    checkpoint = torch.load(pretrained_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])

    print("Loaded CIFAR-10 pretrained model:", pretrained_path)
    print("Pretrained best validation accuracy:", checkpoint["best_val_acc"])
    ...
    # Lower LR than normal training because we are fine-tuning existing useful features.
    optimizer = AdamW(
        model.parameters(),
        lr=5e-4,
        weight_decay=1e-4
    ...
    best_model_path = os.path.join(MODELS_DIR, "resnet18_pretrained_finetuned_kl_best.pth")

    print("\nStarting CIFAR-10 pretrained → CIFAR-10H KL fine-tuning...")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if val_kl < best_val_kl:
            ...
            torch.save(
                {
                    ...
                    "head_type": "linear",
                    "loss": "KL",
                    "initialization": "CIFAR10_HARD_LABEL_PRETRAINED",
            ...
            print("Saved best fine-tuned model:", best_model_path)
    ...
    print("Best validation KL:", best_val_kl)
