...
def plot_entropy_scatter(true_entropy, pred_entropy):
    ...
    plt.title("Predicted vs True Entropy - Custom KL + Entropy Model")
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "custom_predicted_vs_true_entropy.png")
...
def main():
    ...
    model_path = os.path.join(MODELS_DIR, "resnet18_custom_best.pth")

    checkpoint = torch.load(model_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])

    print("Loaded model:", model_path)
    print("Best validation custom loss:", checkpoint["best_val_loss"])
    print("Best epoch:", checkpoint["epoch"])
    print("Lambda entropy:", checkpoint["lambda_entropy"])
    ...
    results = {
        "model": "ResNet18_Custom_KL_Entropy",
    ...
    save_csv = os.path.join(RESULTS_DIR, "custom_test_results.csv")
    ...
    pred_save_path = os.path.join(RESULTS_DIR, "custom_test_predictions.npz")
    np.savez(
        pred_save_path,
        indices=indices,
