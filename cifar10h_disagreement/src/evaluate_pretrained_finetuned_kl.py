...
def plot_entropy_scatter(true_entropy, pred_entropy):
    ...
    plt.title("Predicted vs True Entropy - CIFAR-10 Pretrained Fine-tuned KL")
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "pretrained_finetuned_kl_predicted_vs_true_entropy.png")
...
def main():
    ...
    model_path = os.path.join(MODELS_DIR, "resnet18_pretrained_finetuned_kl_best.pth")

    checkpoint = torch.load(model_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])

    print("Loaded model:", model_path)
    print("Best validation KL:", checkpoint["best_val_kl"])
    print("Best epoch:", checkpoint["epoch"])
    print("Initialization:", checkpoint["initialization"])
    ...
    results = {
        "model": "ResNet18_CIFAR10_Pretrained_Finetuned_KL",
    ...
    save_csv = os.path.join(RESULTS_DIR, "pretrained_finetuned_kl_test_results.csv")
    ...
    pred_save_path = os.path.join(RESULTS_DIR, "pretrained_finetuned_kl_test_predictions.npz")
    np.savez(
        pred_save_path,
        indices=indices,
