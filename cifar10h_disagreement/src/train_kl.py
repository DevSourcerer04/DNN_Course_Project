...
def plot_training_curves(train_losses, val_losses, val_entropy_mae):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("KL Loss")
    plt.title("KL Training and Validation Loss")
    plt.legend()
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "kl_training_validation_loss.png")
    ...
    plt.title("Validation Entropy Error Across Epochs")
    plt.legend()
    plt.tight_layout()

    save_path = os.path.join(PLOTS_DIR, "kl_validation_entropy_mae.png")
    plt.savefig(save_path, dpi=300)
    plt.close()
    print("Saved:", save_path)
...
def main():
    ...
    best_model_path = os.path.join(MODELS_DIR, "resnet18_kl_best.pth")

    print("\nStarting KL training...")
